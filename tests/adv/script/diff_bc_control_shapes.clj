;; diff_bc_control_shapes.clj -- control-flow quad parity.

(load-file "tests/adv/edge_helpers.clj")
(load-file "tests/adv/gen.clj")
(load-file "tests/adv/gen_bc_control_shapes.clj")
...
(def diff-control-seed-tag 0xC0E7E0)
(seed! (bit-xor effective-seed diff-control-seed-tag))

(let [start (now-ms)
      results (atom {:passed 0 :failed 0})]
  (dotimes [i n-progs]
    (let [program (gen-control-shape)
          tmp     (str "/tmp/mino-tests-diff-ctrl-" i ".clj")
          _       (spit tmp program)
      ...
      (if (quad-byte-identical? quad)
        ...
        (let [shrunk (try (shrink-divergent mino-bin lean-bin program
                                            {:budget-ms 10000})
                          (catch e program))
              rfile (str "tests/adv/regressions/diff-ctrl-"
                         effective-seed "-" i ".clj")]
          (try
            (spit rfile
                  (str ";; quad-divergence; control-shape probe.\n"
          ...
          (emit-verdict "diff-ctrl.divergence" "fail"
                        :i i :seed effective-seed :rfile rfile)))))
  (let [r @results]
    (emit-verdict "diff-ctrl.summary"
    ...
    (when (pos? (:failed r))
      (throw (ex-info "diff-ctrl: divergences"
                      {:probe "diff-ctrl" :failed (:failed r)})))))
