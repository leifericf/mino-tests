;; diff_bc_closure_shapes.clj -- closure-capture quad parity.

(load-file "tests/adv/edge_helpers.clj")
(load-file "tests/adv/gen.clj")
(load-file "tests/adv/gen_bc_closure_shapes.clj")
...
(def diff-closure-seed-tag 0xC10FE)
(seed! (bit-xor effective-seed diff-closure-seed-tag))

(let [start (now-ms)
      results (atom {:passed 0 :failed 0})]
  (dotimes [i n-progs]
    (let [program (gen-closure-shape)
          tmp     (str "/tmp/mino-tests-diff-cs-" i ".clj")
          _       (spit tmp program)
      ...
      (if (quad-byte-identical? quad)
        ...
        (let [shrunk (try (shrink-divergent mino-bin lean-bin program
          ...
          (try
            (spit rfile
                  (str ";; quad-divergence; closure-shape probe.\n"
  ...
  (let [r @results]
    (emit-verdict "diff-closure.summary"
    ...
    (when (pos? (:failed r))
      (throw (ex-info "diff-closure: divergences"
                      {:probe "diff-closure" :failed (:failed r)})))))
